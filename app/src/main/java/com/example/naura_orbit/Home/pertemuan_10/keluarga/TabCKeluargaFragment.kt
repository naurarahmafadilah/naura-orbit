...
class TabCKeluargaFragment : Fragment() {
    ...
    private val fullDataKeluarga = mutableListOf<KeluargaModel>()
    private val displayedDataKeluarga = mutableListOf<KeluargaModel>()
    private lateinit var keluargaAdapter: KeluargaAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (fullDataKeluarga.isEmpty()) {
            fullDataKeluarga.addAll(listOf(
        ...
        displayedDataKeluarga.clear()
        displayedDataKeluarga.addAll(fullDataKeluarga)

        keluargaAdapter = KeluargaAdapter(displayedDataKeluarga) { keluarga ->
        ...
        binding.rvKeluargaTabC.adapter = keluargaAdapter
    }

    fun tambahKeluargaBaru(keluarga: KeluargaModel) {
        fullDataKeluarga.add(0, keluarga)
        displayedDataKeluarga.add(0, keluarga)
        if (::keluargaAdapter.isInitialized) {
            keluargaAdapter.notifyItemInserted(0)
            binding.rvKeluargaTabC.scrollToPosition(0)
        }
    }
    ...
    fun performSearch(query: String) {
        ...
        if (cleanQuery.isEmpty()) {
            ...
            val filtered = fullDataKeluarga.filter {
                it.noKK.lowercase().contains(cleanQuery) ||
                it.kepalaKeluarga.lowercase().contains(cleanQuery) ||
                it.alamat.lowercase().contains(cleanQuery)
            }
            displayedDataKeluarga.addAll(filtered)
        }
        if (::keluargaAdapter.isInitialized) {
